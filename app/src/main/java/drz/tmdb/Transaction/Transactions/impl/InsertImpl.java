...
public class InsertImpl implements Insert {
    ...
    private Integer insert(int classId, List<String> columns, Tuple tuple, int l, int[] index) throws TMDBException {
        ...
        List<Integer> trajDeputyClassIds = tJoinDeputySize(classId);
        //如果list非空就需要进一步判断
        if (!trajDeputyClassIds.isEmpty()) {
            ...
            for (int deputyId : trajDeputyClassIds) {
                //得到当前代理类的id
                //由于基于轨迹相似度的代理类需要两个源类，需要拿到另一个源类，这里使用getAnotherDeputy方法拿到另一个源类的id
                int anotherClassId = getAnotherDeputy(deputyId, classId);
                //通过select的getTable（classid）方法拿到另一个源类的所有tuple
                TupleList otherClassTuples = select.getTable(anotherClassId);
                //遍历另一个源类的所有tuple
